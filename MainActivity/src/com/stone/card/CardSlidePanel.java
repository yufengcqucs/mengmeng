...
public class CardSlidePanel extends ViewGroup {
	...
	private View leftBtn, rightBtn, loveBtn;
		...
		btnListener = new View.OnClickListener() {
			...
			public void onClick(View view) {
				if (view.getId() == R.id.maskView) {
					...
					Intent intent = new Intent(finalContext, ShowDetailInfoActivity.class);
					finalContext.startActivity(intent);

				} else if (view.getId() == R.id.likeperson_btn) {
				...
				} else if (view.getId() == R.id.callmessage_btn) {
					// TODO 执行聊天程序
				} else if (view.getId() == R.id.sendflower_btn) {
					// TODO 执行鲜花程序
				}
			}
		};
	...
	protected void onFinishInflate() {
		...
		for (int i = num - 1; i >= 0; i--) {
			...
			if (childView.getId() == R.id.card_bottom_layout) {
				...
				loveBtn = viewItem.findViewById(R.id.likeperson_btn);
				loveBtn.setOnClickListener(btnListener);
			}
		}
	...
	private void initBottomLayout() {
		leftBtn = bottomLayout.findViewById(R.id.callmessage_btn);
		rightBtn = bottomLayout.findViewById(R.id.sendflower_btn);
	...
	public interface CardSwitchListener {
		 ...
		 *            飞向两侧的卡片数据index
		 * @param type
		 *            飞向哪一侧{@link #VANISH_TYPE_LEFT}或{@link #VANISH_TYPE_RIGHT}
		 */
